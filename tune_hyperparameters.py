...
class Dashboard:
    ...
    def render(self, force=False):
        ...
        print(
            f" 总体进度: {len(self.completed_runs)}/{self.total_configs} 已完成 | 已耗时: {self.elapsed:.1f}秒 | 预计剩余: {rem_str}"
        )
        print("=" * 85)
        print()
        ...
        if not self.completed_runs:
            ...
            for rank, r in enumerate(sorted_runs):
                ...
                print(
                    f"| {rank + 1:<4} | {label_trunc:<25} | {r['lr']:<9} | {r['model_epochs']:<9} | {r['opt_steps']:<9} | {r['rms_acc']:<10.4f} | {r['rms_tire']:<10.4f} | {r['eval_cost']:<8.4f} |"
                )
        print("-" * 85)
        print()
        ...
        if len(self.completed_runs) < self.total_configs:
            ...
            print(
                f" 参数: lr={self.current_lr} | model_epochs={self.current_epochs} | opt_steps={self.current_steps} | num_trials={self.current_trials}"
            )
            print(f" 状态: {self.current_status} {self.current_trial_str}")

            if self.current_status == "控制策略更新":
                print(
                    f" 进度: [梯度下降] 步数 {self.current_opt_step:3d}/{self.current_steps} | 当前Cost: {self.current_opt_cost:.4f}"
                )
...
def run_experiment(script_path, run_name, args_dict, db, unknown_args=None):
    ...
    if unknown_args:
        # 过滤掉 args_dict 中已指定的参数，避免被 unknown_args (如网页端带入的静态默认值) 覆写
        keys_to_remove = set(args_dict.keys())
        filtered_unknown = []
        i = 0
        while i < len(unknown_args):
            arg = unknown_args[i]
            if arg.startswith("-"):
                clean_key = arg.lstrip("-")
                if clean_key in keys_to_remove:
                    i += 1
...
def main():
    ...
    parser.add_argument(
        "-script",
        type=str,
        default="test_mcpilco_quarter_car_gym_residual.py",
        choices=[
    ...
    parser.add_argument(
        "-sweep_mode", type=str, default="quick", choices=["quick", "grid", "custom"], help="Sweep plan mode."
    )
    parser.add_argument("-custom_configs", type=str, default=None, help="JSON list of custom configuration dicts.")
    parser.add_argument(
        "-no_tui", action="store_true", help="Disable clearing screen and printing interactive TUI dashboard."
    )
        ...
        try:
            ...
            for idx, cfg in enumerate(sweep_configs):
                ...
                if "label" not in cfg:
                    cfg[
                        "label"
                    ] = f"custom_lr{str(cfg['lr']).replace('.', 'p')}_epochs{cfg['model_epochs']}_opt{cfg['opt_steps']}"
    ...
    for idx, r in enumerate(sorted(db.completed_runs, key=lambda x: x["eval_cost"])):
        print(
            f"| {idx + 1:<4} | {r['label']:<25} | {r['lr']:<9} | {r['model_epochs']:<9} | {r['opt_steps']:<9} | {r['rms_acc']:<14.4f} | {r['rms_tire']:<14.4f} | {r['eval_cost']:<8.4f} |"
        )
    print("#" * 85 + "\n")
